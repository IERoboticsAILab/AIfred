...
def pose_callback(msg):
    ...
    global initial_yaw, last_yaw, accumulated_rotation
    global playlist_index, videos
    
    # Check height for pause gesture
    height = msg.pose.position.z
    if abs(height) > 0.9:
        rospy.loginfo("Lift gesture detected - toggling pause")
        run_xdotool(['search', '--onlyvisible', '--class', 'chrome', 
                    'windowactivate', '--sync', 'key', 'space'])
        cooldown = 2.0
        time.sleep(cooldown)
...
if __name__ == '__main__':
    try:
        ...
        rospy.Subscriber("/natnet_ros/umh_5/pose", PoseStamped, 
                        callback=pose_callback, queue_size=1)
        rospy.Subscriber('/urls_to_open', UrlToOpen, 
                        callback=urls_callback, queue_size=1)
